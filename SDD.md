...
- **`SitePaymentMethod` belongs to the Site, not the User** (§4). Decision and rationale
  recorded in full in `docs/archive/phase3-saved-payment-method-plan.md`; summary: this is a
...
- **Step-up re-authentication gates reuse, not addition.** `POST /s/:token/authorize`
  ...
  `docs/archive/phase3-saved-payment-method-plan.md` for why no just-added exception exists.
...
step of the same complexity-reduction pass as #036/#037.

- Archived the 10 phase-plan markdown files under `docs/` (all superseded --
  each one's content is already absorbed into this spec's numbered sections
  and/or the changelog above) plus `docs/phase3-smoke-test-report.md` (a
  point-in-time manual-verification record, kept for provenance rather than
  deleted) into new `docs/archive/`. Updated the two live (non-changelog)
  references to `docs/phase3-saved-payment-method-plan.md` -- §9.4 above and
  a comment in `src/db/schema.ts` -- to the new `docs/archive/` path.
  Historical changelog entries referencing the old `docs/` path (there are
  none below this point that do) are left as accurate records of their time,
  per this file's established convention.
- Split this Changelog: entries #001-#016 and the "Pre-convention history
  (PR #1-#13)" section moved to new `docs/archive/changelog-archive.md`,
  verbatim, with a pointer left in their place. The changelog had grown to
  outweigh the spec body it documents (62% of the file vs. 38%); this trims
  the file every future edit touches without losing anything -- the archive
  follows the same numbering/format convention and is not itself a second
  place to look first.

No `src/` change; `npm run lint` (prettier covers `.md`) confirms formatting.
...
Entries #001–#016 and the "Pre-convention history (PR #1–#13)" section that
followed them were moved to `docs/archive/changelog-archive.md` (SDD.md
changelog entry #038) to keep this file's working size down. They're
